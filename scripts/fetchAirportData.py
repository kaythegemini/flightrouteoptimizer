...
def filter_by_region(airports, region: str, country_code: str) -> list:
    """_summary_
    ...
    for airport in airports:
        if airport['continent'] == region and airport['country'] == country_code:
...
def main():
    ...
    try:
        ...
        sl_ap = our_airports.getAirportsByType('small_airport')
    except:
    ...
    airports_data = parse_airport_data(med_ap) + parse_airport_data(lg_ap) + parse_airport_data(sl_ap)

    # filter for north america and some central america
    ap_filt = filter_by_region(airports_data, "AS", "IN")
